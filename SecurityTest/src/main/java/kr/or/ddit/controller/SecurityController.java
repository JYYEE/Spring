...
public class SecurityController {
	 ...
	 * 		- CSRF 토큰 처리 (보안 강화)
	 * 		- 암호화 처리
	 * 		- 자동 로그인 (remeberme라는 키를 가지는 쿠키값으로 저장)
	 * 
	 * 			** CSRF 용어 설명
	 ...
	 *		- 일반 게시판 등록 화면 (화면 권한을 가진 사용자만 접근 가능 : hasRole('ROLE_MEMBER'))
	 *			> 접근 제한이 걸려 스프링 시큐리티가 기본적으로 제공하는 로그인 페이지로 이동합니다.
	 *		- 공지사항 게시판 목록 화면 (모두 접근 가능하도록 되어 있습니다 : permitAll)
	 *		- 공지사항 게시판 등록 화면 (화면 권한을 가진 사용자만 접근 가능 : hasRole('ROLE_ADMIN'))
	 *			> 접근 제한이 걸려 스프링 시큐리티가 기본적으로 제공하는 로그인 페이지로 이동합니다.
	 *
	 * 4. 로그인 처리
	 * 	- 메모리 상에 아이디와 패스워드를 지정하고 로그인을 처리합니다.
	 ...
	 * 					프로젝트를 만들고 설정하는 과정에서 base-package를 나눠야 하는 상황일때는 위와 같은 설정이 필요할 수 있다.
	 ...
	 * 					: 빈 등록에 사용되는 클래스는 security 패키지 아래에 있는 CustomAccessDeniedHandler 클래스
	 * 
	 * 		접근 거부 처리자 클래스 정의
	 * 		- 사용자가 정의한 접근 거부 처리자
	 ...
	 * 			> 접근 제한에 걸려 스프링 시큐리티가 제공하는 로그인 페이지가 나타나고,
	 * 				회원 권한을 가진 계정으로 접근 시, 공지사항 게시판 등록 페이지는 관리자 권한만 접근 가능하므로
	 * 				접근이 거부된다. 이 때, access-denied-handler로 설정되어 있는 ref 속성에 부여된 클래스 메소드로 이동하고
	 * 				해당 페이지에서 접근이 거부되었을 때 보여질 페이지의 정보가 나타난다.
	 * 
	 * 7. 사용자 정의 로그인 페이지
	 ...
	 * 		- 스프링 시큐리티 설정
	 * 			> security-context.xml 설정
	 * 				- <security:form-login/> 삭제 (기본 로그인 페이지로 사용할 때 필요. 사용자 정의 로그인 페이지를 사용하기 때문에 주석처리)
	 ...
	 * 				인증이 완료되면 최초의 요청된 target URI로 이동합니다. 그렇지 않은 경우 사용자가 만들어 놓은
	 * 				접근 거부 페이지로 이동합니다.
	 * 
	 * 8. 로그인 성공 처리
	 ...
	 * 				넘겨받은 파라미터들 중 authentication 안에 principal로 User 정보를 받아서 username과 password를 출력한다.
	 ...
	 * 		환경 설정
	 * 		- 의존 라이브러리 설정
	 * 			> 데이터 베이스 관련 라이브러리를 추가한다.
	 * 			> 기존 데이터베이스 연결을 위한 라이브러리가 추가 되어 있다. (SecurityTest 프로젝트 안에는 없기 때문에 새롭게 등록)
	 * 
	 * 		스프링 설정
	 * 		- root-context.xml 설정
	 * 			> 데이터 소스 설정(기존 설정)
	 ...
	 * 				그래서 암호화를 하지 않는 PasswordEncoder를 직접 구현하여 지정하면 로그인 시 암호화를 고려하지 않으므로 로그인이 정상적으로 이루어지는 걸 확인할 수 있습니다.
	 ...
	 * 			- encode() 메서드를 통해서 SHA-2 방식의 8바이트 Hash 암호를 매번 랜덤하게 생성합니다.(SHA-1방식은 이미 뚫림)
	 * 			- 똑같은 비밀번호를 입력하더라도 암호화되는 문자열은 매번 다른 문자열을 반환한다.
	 * 
	 * 		> 비밀번호를 입력하면 암호화된 비밀번호로 인코딩되는데, 암호화된 비밀번호와 디비 테이블에 있는 암호화된 비밀번호가 일치한지를 파악한 후 ,
	 * 		    일치하면 로그인 성공으로 다음 스텝을 진행합니다. 
	 * 			- BCryptPasswordEncoder 클래스의 encode() 메소드를 통해 만들어지는 암호화된 해쉬 다이제스트들은 입력한 비밀번호 문자에 해당하는
	 * 				수십억개의 다이제스트들 중에서 일치하는 다이제스트가 존재할 경우 비밀번호의 일치를 보고 인증을 성공시켜 줍니다.  
	 *
	 ...
	 * 			> CustomUserDetailsService 클래스 내 loadUserByUsername 메소드에서 인코딩된 비밀번호를 확인해보고 
	 * 				암호화된 비밀번호가 어떻게 일치하여 인증이 되는지를 같이 알아보도록 한다. 		
	 * 
	 * 13. 스프링 시큐리티의  표현식
	 ...
	 *    기능을 구현하기 위해 <security:remember-me> 태그를 사용하여 시큐리티 설정 파일을 수정한다.
	 *    
	 *    	데이터베이스 테이블 
	 ...
	 * 		- JSESSIONID를 삭제 후, 다시 로그인을 진행하더라도 로그인 후 진행될 페이지가 정상적으로 나타나는 걸 확인할 수 있습니다.
	 ...
	 *  	- @PostAuthorize
	 *  		> 메서드가 실행된 후에 적용할 접근 정책을 지정할 때 사용한다. 		
	 */	
}
