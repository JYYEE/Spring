...
public class TransactionController {
	 ...
	 * 			삭제, 수정도 마찬가지로 이루어진다.
	 * 
	 *  	@Transactional 을 적용하고 중간에 에러를 발생시킨 후, 롤백된 상태를 확인하려고 했으나 롤백이 되지 않고 데이터베이스에 데이터가 등록된다.
	 *  	-> 그 이유는 스프링 프레임워크에서 @Transactional 어노테이션은 기본적으로 Checked Exception에 대해서는 롤백 처리를 하지 않도록 설계되어 있다.
	 *  		기본적으로 스프링에서 트랜잭션 처리는 RuntimeException 계열이라면 Rollback처리를 한다.  
	 *  		그러나 UncheckedException에 대해서는 Rollback처리를 진행하지 않는다.  
	 *  		(구글링하는 정보만 볼 게 아니라 공식 문서를 꼭 확인해볼것!!)
	 *  
	 ...
	 * 			수두록하여 많은 에러 정보를 공유하고 있다.(모든 블로그가 그렇진 않지만, 대표적인 표가 무수히 많이 공유되고 있다.)
	 * 
	 * 		*** 
	 * 		'예외처리 시 트랜잭션 처리'에 대한 정보가 잘못되어 표기된 경우가 많다.
	 * 		이럴때 되고 저럴때 되고가 아니라, 기본적인 스프링 트랜잭션의 정책이 있고 그 안에서 정책이 외에 옵션으로 특정 에러가 발생했을 때
	 * 		롤백이 진행할 수 있도록 제공한다. (많은 블로그들을 100% 믿지 말기 바랍니다... 혹여나 블로그를 작성한다면 검증된 내용으로만 적어주길 바람!!)
	 * 		
	 ...
	 * 		- RuntimeException의 상속 여부에 따라서 Checked Exception과 UnChecked Exception 으로 나뉘어진다. 
	 ...
	 * 						> 하나의 트랜잭션 처리가 이루어진 변경 내용이 Commit된 후, 다른 트랜잭션에서 조회가 가능합니다.
	 ...
	 * 						> 트랜잭션 Commit 이 일어난 데이터에 대해서 조회가 가능 (트랜잭션 완료시까지 조회에 대한 Shared lock이 걸리지 않음)
	 * 				- SERIALIZABLE : Dirty read, Non-repeatable Read 및 Phantom Read가 방지됨을 나타내는 상수입니다. 
	 * 						> Phantom Read가 발생하지 않는다. 
	 * 						> 거의 사용하지 않는 옵션 *******
	 * 
	 * 			2) propagation (전파 옵션)
	 * 				- 기존 진행중인 트랜잭션 외에 추가적인 진행중인 트랜잭션이 존재할 때 추가적인 트랜잭션에 대해서 어떻게 처리할 지에 대한 설정
	 * 				- 추가적인 트랜잭션을 기존 트랜잭션에 포함시켜 함께 처리할 수 도 있고, 추가적인 트랜잭션처럼 별도의 트랜잭션으로 추가할 수 도 있고
	 * 					다른 트랜잭션처럼 진행되다 에러를 발생시킬 수도 있다. 
	 * 
	 * 				2-1) 옵션
	 * 					- REQUIRED : 현재 트랜잭션을 지원하고 존재하지 않는 경우 새 트랜잭션을 만듭니다.
	 * 						> propagation 기본 default 옵션에 해당합니다.
	 * 						> 부모/자식 간에  상관관계에서 자식 부분의 트랜잭션이 rollback 처리시, 부모까지 영향이 가서 rollback 처리 된다.
	 ...
	 * 			4) timeout (트랜잭션 제한시간)
	 * 				- 기본값은 -1로 무제한입니다.
	 * 				- timeout은 클라이언트와 서버와의 통신 중, 서버측 문제로 다음 처리를 이어나가지 못하는 'Deadlock'을 방지할 수 있는 속성입니다. // 적정시간을 줘야 무한 로딩을 가지지 않고, 서버와 연결을 끊어서 다시 요청할 수 있는 환경 만들어줌.
