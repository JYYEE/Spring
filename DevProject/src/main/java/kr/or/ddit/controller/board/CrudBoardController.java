...
public class CrudBoardController {
	...
	// 프록시가 interface, class 기반인지 확인할 수 있음
	@PostConstruct
	public void init() {
		log.info("aopProxy 상태(interface기반) : {} " + AopUtils.isAopProxy(service));	// 예상 결과 : true
		log.info("aopProxy 상태(class기반) : {} " + AopUtils.isCglibProxy(service));	// 예상 결과 : false
	}
