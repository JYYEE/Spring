...
public class JSPFmttagController {
	...
	// 만약 type 속성이 percent인 경우엔 넘겨받아야 할 값이 '1000%'과 같이 %의 스타일 형태 값이 넘어가야 한다.
	// 그래서 type속성과 일치하여 값을 파싱할 수 있다. 
	@RequestMapping(value = "/home0202", method = RequestMethod.GET)
	public String home0202(Model model) {
		String coin = "￦1000"; // 값이 %를 포함한 값일 때는 type은 percent로 주기
		model.addAttribute("coin", coin);
	...
	// 6) type 속성을 date로 지정하여 날짜 포맷팅 한다.
	...
	// 7) type 속성을 time로 지정하여 시간 포맷팅 한다.
	...
	// 11) dateStyle 속성으로 short로 지정하여 문자열을 Date 객체로 변환한다. 
	@RequestMapping(value = "/home0402", method = RequestMethod.GET)
	public String home0402(Model model) {
		String dateValueS = "20. 2. 1"; // style short 형태로 지정
		String dateValueM = "2020. 2. 1";	// style medium 형태로 지정
		String dateValueL = "2019년 2월 1일 (금)";	// style long 형태로 지정
		String dateValueF = "2019년 2월 1일 금요일";	// style full 형태로 지정
		// 각 dateStyle로 지정된 값이 페이지로 넘어가 parsing이 진행될 때, 
		// parseDate내에 dateStyle을 각 값과 일치하는 스타일 형태로 지정해주어야 값이 파싱된다.
		model.addAttribute("dateValueS", dateValueS);
		model.addAttribute("dateValueM", dateValueM);
		model.addAttribute("dateValueL", dateValueL);
		model.addAttribute("dateValueF", dateValueF);
