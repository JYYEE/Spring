...
public class DatabaseConnectController {
	 ...
	 *  	> 마이바티스
	 *  
	 *  	** 우리는 마이바티스를 이용한 CRUD게시판을 진행하도록 합니다.
	 *  	
	 ...
	 * 		3) 게시판 구현
	 * 		
	 * 		** 기본적인 JDBC를 이용한 DB설계는 DAO란에서 sql 쿼리가 노출되어 개발하는 방식이다보니 우리가 중급프로젝트때 
	 * 		또는 JSP에서 preparestatement방식의 데이터베이스 연동과 다를 바 없는 형태로, 게시판 구현을 진행하지 않고 내용만 간략히 보고 넘어가도록 한다.
	 * 
	 ...
	 * 
	 * 		#[장점]
	 * 		- SQL위주의 쿼리를 작성하지 않아도 된다. // 메소드로 sql을 요청
	 * 		- 코드량이 엄청 줄어든다.(생산성 증가)
	 * 		- 객체 위주의 코드 작성으로 가독성이 좋아진다. (생산성 증가)
	 ...
	 * 		
	 * 		# JPA관련 어노테이션
	 * 		자바빈즈 클래스 객체가 곧 테이블 구조와 같은 형태이기 때문에, VO 클래스 내 @Entity어노테이션을 통해 해당 객체가 테이블과 같은 Entity임을
	 * 		설정하고 @Table어노테이션으로 테이블명을 설정해주면 VO 클래스는 데이터베이스의 생성되어 있는 테이블과 연결되기 위한 준비가 된다.
	 * 		ex) 	
	 * 			@Entity
	 * 			@Table(name="board")
	 * 			public class Board { ...}
	 ...
	 * 		자동으로 Entity를 분석하여 함수에 따른 쿼리를 자동 생성하여 결과를 실행, 조회, 수정 등을 처리해준다.
	 * 		기존 SQL 쿼리를 이용하여 서비스를 처리할 때에 필드가 추가되면 필드와 관련되어 있는 기능, sql, 추가된 필드 등을 수정해야하는 번거로움이 발생하지만 
	 * 		JPA는 필드만 추가 후 테이블 정보와 매핑만 시켜주면 JPA가 자동으로 분석을 진행한다.  
	 * 		그리고, 사용하는 데이터베이스 종류가 변경된다 하더라도 문제가 없다. (JPA는 SQL을 직접 건들이지 않고 함수와 같은 기능들로만 쿼리를 요청하고 데이터베이스와 통신하기 때문입니다.)
	 * 
	 ...
	 *  	객체모델을 이용하여 비즈니스 로직을 구성하는데에만 집중을 할 수 있다. 하지만, 프로젝트의 규모와 복잡도가 클 때는 설계가 잘못되는 경우 해당 프로젝트의 품질이 떨어진다는 문제가 있습니다.
	 ...
	 * 		- 저는 ibatis를 사용했습니다. 또는 저는 mybatis를 사용했습니다. 와 같은 대답을 하시면 됩니다.
	 * --------------------------------
	 * 
	 * 7. 마이바티스
