...
public class HomeController {
	...
	public String goHome0201() {
		...
		return "home0201"; // return 값이 주소 경로가 됨.
	}
	 ...
	 * 
	 * (1) ResponseBody를 이용하는 방식(controller와 같이 사용)
	 * (2) RestController를 이용하는 방식(controller만 사용)
	 * 
	 * @ResponseBody를 지정하지 않으면 HTTP 404에러가 발생한다.(jackson-databind 라이브러리가 설정되어 있지 않는 경우와 xml 라이브러리 주석처리 안한것도 포함)
	 ...
	 */
	// 응답을 할 때 바디에 데이터를 넘길 때 사용
	@ResponseBody // 객체를 리턴할 때 필요한 어노테이션 -> 객체가 json 타입으로 반환되기 때문에 pom.xml에서 json데이터 바인딩을 위한 라이브러리 주석 해제, xml데이터 바인딩을 위한 라이브러리는 주석 
	...
	// Network > Headers > Status Code > 200 OK 상태코드를 전송한다.
	// Void 타입은 아무런 형태가 아닌데 제네릭 타입의 뭔가는 채워야겠어서 채우는 placeholder 같은 느낌
	...
	public ResponseEntity<Void> goHome0601(){
		...
		// 해당 URL 요청 후, 브라우저에서 개발자 도구 이용해서 네트워크 탭을 확인해보면 응답으로 URL이 응답으로 나간 걸 확인할 수 있는데,
		// 이 때, 상태코드 200으로 정상 응답이 나간 걸 확인할 수 있다.
		// 그리고, 다른 기능으로 아무런 형태 없이 응답으로 나가지만 응답에 대한 header정보를 변경하고자 할 때 사용할 수 있다.
	}
	/*
	 * 7) ResponseEntity<String> 타입
	 * - response 할 때 HTTP헤더 정보와 문자열 데이터를 전달하는 용도로 사용한다.
	 * 
	 * (1) ResponseBody를 이용하는 방식
	 * (2) RestController를 이용하는 방식
	 */
	...
	/*
	 * 11) ResponseEntity<byte[]> 타입 (파일 전송 및 다운)
	...
	public ResponseEntity<byte[]> goHome1101(){
		...
		try {
			// 파일 경로를 나타내는 방법 2가지
			// 방법 1('\\'_
			// in = new FileInputStream("D:\\A_TeachingMaterial\\6.JspSpring\\02.SPRING2\\ccc.jpg");
			// 방법 2 ('/')
			in = new FileInputStream("D:/A_TeachingMaterial/6.JspSpring/02.SPRING2/jjj.gif");
			// 서버 내부의 경로로 설정하고 파일 이름 설정하면 썸네일을 확인할 수 있다.
			headers.setContentType(MediaType.IMAGE_JPEG);
			// header에 컨텐트 타입이 image_jpeg라는 걸 설정
			entity = new ResponseEntity<byte[]>(IOUtils.toByteArray(in), headers, HttpStatus.CREATED);
			// fileData를 created하겠다는 의미. IOUtils는 commons.io 라이브러리를 update하고 나서 사용가능.
		} catch (Exception e) {
			e.printStackTrace();
	...
	// 파일의 데이터를 브라우저가 다운로드 받도록 한다.(파일 다운)
	@ResponseBody
