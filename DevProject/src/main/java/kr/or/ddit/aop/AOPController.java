...
// @Aspect는 어노테이션을 붙여 이 클래스가 Aspect를 나타내는 클래스라는 걸 명시 (@Component와 짝궁)
// AOPController 클래스 빈 등록 시, aOPController로 할지 aoPController로 할지 명확하지 않을 수 있어서 aopController라는 이름을 명시해줌
...
public class AOPController {
	 ...
	 * 
	 * 		[AOP란? 예시]
	 * 		406호 반장 정수 학생이 신입으로 프로젝트를 진행하고 있습니다.
	 * 		그러던 어느날, 성희 팀장님을 통해서 '반장님, 지금 개발중인 서비스 처리 속도 좀 로그로 남겨주세요!' 라는 부탁을 받습니다.
	 ...
	 * 		성희 팀장님은 아주 긍정적인 검토안을 정수 학생한테 전달하면서 '우리 서비스 전체에도 각 처리속도를 찍어주세요'라고 다시 부탁합니다.
	 * 		정수 학생은 본인이 만들어 낸 메소드를 각 기능별 서비스 로직에 하나하나씩 약 2만개쯤 넣을 쯤에 의문을 갖습니다...
	 * 
	 * 			'서비스 로직에서 제일 중요한 로직은 본래의 기능이 제일 중요하고 지금 내가 작성하는 로직은 옵션(부가 기능)이 추가되는게 아닐려나?'
	 * 			'그럼 이걸 하나의 묶음으로는 처리가 불가능한건가?'
	 * 
	 * 		하는 생각을 하게 됩니다. 그렇게 정수 학생은 위와 같은 고민을 해결하기 위해서 구글링을 시작합니다. 
	 * 
	 ...
	 * 			- 횡단 관심사 분리 (Separation Of Cross-Cutting Concern) : 횡단 관심사에 해당하는 부분(메소드 실행시작시간 출력, 메소드 처리 후 시간 출력 등)을
	 * 																분리해서 한 곳으로 모으는 것을 의미
	 * 			- @Component : @Aspect와 짝궁. component-scan시 "저 여기 있어요! 여기 봐주세요!" 라는 의미
	 ...
	 *				> 어디에 적용할 것인지 결정, 메소드? 객체 생성시? 필드 접근시? 등등
	 *			- 어드바이스 (Advice)
	 ...
	 * 		
	 * 		AOP는 서비스 내의 비즈니스 로직을 가로채서 컨트롤을 하는데, 이 때 프록시라는 녀석을 이용해서 해당 기능을 실행하기 전 후 또는 예외 시에
	 ...
	 *  
	 */
	//프록시는 타겟으로 위장해서 타겟 대신 요청을 받아 중복 메소드를 먼저 실행 후에 서비스로 보내서 주 기능인 비즈니스 로직을 처리해주도록 함.
	@Before("execution(* kr.or.ddit.service.IBoardService.*(..))") // IBoardService가 proxy로 감싸고 있어서 IBoardService를 들어가려면 aopController를 거쳐서 들어감
	public void startLog(JoinPoint jp) {
	 ...
	 * ** ex) crud board에서 delete부를 에러로 실행해봅시다.( 쿼리를 no =  를 no2 = 으로 변경해서 진행)
	 */
	 ...
	 * 책임이 around 어드바이스로 전개되고 그래서 비즈니스 로직에 대한 정보를 around 어디바이스 메소스가 가지고 있어야 하고
