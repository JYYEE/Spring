...
<!-- 태그라이브러리는 jsp 페이지 안에서 실행되므로 페이지마다 선언 -->
		...
		<div class="col-md-12">
			<div class="card card-primary">
				<form action="/notice/insert.do?${_csrf.parameterName }=${_csrf.token}" method="post" id="noticeForm" enctype="multipart/form-data">
					<c:if test="${status eq 'u' }">
						<input type="hidden" id="boNo" name="boNo" value="${notice.boNo }">
					</c:if>
...
$(function(){
	CKEDITOR.replace("boContent", {
		footnotesPrefix : "a", // a태그에 대한 속성
		filebrowserUploadUrl : "/imageUpload.do?${_csrf.parameterName }=${_csrf.token}" // 이미지 업로드 후 본문 내용에 적용. ckeditor 이미지 속성에 업로드 탭 생성.
	});
